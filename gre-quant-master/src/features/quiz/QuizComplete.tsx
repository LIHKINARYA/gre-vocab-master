...
export function QuizComplete({ correctCount, total, missed, onRetakeMissed, onNewQuiz, onExit }: QuizCompleteProps) {
  ...
  const byTopic = missed.reduce<Record<string, number>>((acc, m) => {
    acc[m.problem.topic] = (acc[m.problem.topic] ?? 0) + 1;
    return acc;
  }, {});
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {Object.keys(byTopic).length > 0 && (
        <div className="mb-6">
          <p className="text-sm font-semibold mb-2">Missed by topic</p>
        ...
        <div className="mb-6">
          ...
          <div className="space-y-2">
            {missed.map((m) => (
              ...
              >
                <p className="font-mono text-xs text-(--color-slate) mb-1">{m.problem.subtopic}</p>
                <p className="text-(--color-ink) dark:text-(--color-paper)">{m.problem.prompt}</p>
              </div>
            ))}
          </div>
        </div>
