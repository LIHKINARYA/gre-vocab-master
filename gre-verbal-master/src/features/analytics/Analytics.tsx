  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data.dailySeries} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
              ...
              <Tooltip
                contentStyle={{ backgroundColor: 'var(--color-paper)', border: '1px solid rgba(22,33,61,0.15)', borderRadius: 8, fontSize: 12 }}
              />
              <Legend wrapperStyle={{ fontSize: 12 }} />
              <Line type="monotone" dataKey="reviews" name="Attempts" stroke="#b8862b" strokeWidth={2} dot={false} />
              <Line type="monotone" dataKey="newItems" name="New items" stroke="#3e7c74" strokeWidth={2} dot={false} />
              <Line type="monotone" dataKey="accuracy" name="Accuracy %" stroke="#a34a3a" strokeWidth={2} dot={false} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </motion.div>
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-6">
        ...
        >
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={data.masteryBreakdown} dataKey="value" nameKey="name" innerRadius={45} outerRadius={80} paddingAngle={2}>
      ...
      <motion.div
        ...
        className="rounded-xl border border-(--color-ink)/10 dark:border-(--color-paper)/10 bg-white/60 dark:bg-white/5 p-5"
      >
        <p className="font-display text-lg font-semibold mb-4">Accuracy by question type</p>
        <p className="text-xs text-(--color-slate) mb-3">The lowest bar is where extra practice will help you most.</p>
        <div className="h-56">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data.topicAccuracy} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
              ...
              <Tooltip
                contentStyle={{ backgroundColor: 'var(--color-paper)', border: '1px solid rgba(22,33,61,0.15)', borderRadius: 8, fontSize: 12 }}
                formatter={(value, name, props) => [`${value}% (${props.payload.attempts} attempts)`, name]}
              />
