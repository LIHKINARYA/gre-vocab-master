...
export function Dashboard({ onStartPractice, onStartQuiz, onStartCram }: DashboardProps) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {s.strugglingCount > 0 && (
            <Button variant="rust" onClick={onStartCram}>
              Cram {s.strugglingCount} weak item{s.strugglingCount === 1 ? '' : 's'}
            </Button>
          )}
          ...
          <>
            <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
              ...
              <p className="text-sm font-mono text-(--color-gold)">
                {s.itemsSolved} / {s.totalItems} attempted
              </p>
            </div>
            <ProgressBar value={s.studyPlan.progressPercent / 100} color="var(--color-gold)" />
            <p className="text-xs text-(--color-slate) mt-2">
              {s.studyPlan.itemsRemaining > 0
                ? `Work through ~${s.studyPlan.newItemsPerDay} new items/day to finish the bank before your test`
                : 'All items attempted — focus on reviews and mastering struggling ones'}
            </p>
          </>
        ) : (
          <p className="text-sm text-(--color-slate)">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        <StatCard index={0} label="Total Items" value={s.totalItems} accent="var(--color-ink)" />
        <StatCard index={1} label="Solved" value={s.itemsSolved} accent="var(--color-verdigris)" />
        <StatCard index={2} label="Due Today" value={s.dueToday} accent="var(--color-rust)" />
        <StatCard index={3} label="New Remaining" value={s.newItemsRemaining} accent="var(--color-ink)" />
        ...
        <StatCard index={7} label="Mastered" value={s.itemsMastered} accent="var(--color-verdigris)" />
      </div>
      ...
      >
        <div className="flex justify-between items-center mb-2">
          ...
          <p className="text-sm text-(--color-slate)">
            {s.todayNewItems} / {s.dailyGoal} new items
          </p>
        </div>
        <ProgressBar value={s.dailyGoal > 0 ? s.todayNewItems / s.dailyGoal : 0} color="var(--color-verdigris)" />
      </motion.div>
