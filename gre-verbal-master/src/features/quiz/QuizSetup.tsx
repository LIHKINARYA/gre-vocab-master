...
const specialSources: { id: QuizSource; label: string; hint: string }[] = [
  ...
  { id: 'struggling', label: 'Cram weak items', hint: 'Low ease-factor or repeatedly missed' },
];
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto">
      ...
      <div className="mb-8">
        <label className="flex items-center gap-2 text-sm cursor-pointer">
          ...
          />
          Timed mode — mimics real GRE Verbal pacing
        </label>
      </div>

      {available === 0 ? (
        <p className="text-sm text-(--color-rust) mb-4">
          No items in this pool yet — try a different source, or practice some new items first.
        </p>
      ) : (
