...
function App() {
  ...
  return (
    <div className="min-h-screen bg-(--color-paper) dark:bg-(--color-ink)">
      ...
      {view === 'dashboard' && (
        <Dashboard onStartLearning={() => setView('learning')} onStartCram={() => goToQuiz('struggling')} />
      )}
      {view === 'learning' && <LearningSession onExit={() => setView('dashboard')} />}
      {view === 'quiz' && <QuizMode onExit={() => setView('dashboard')} initialSource={quizSource} />}
      {view === 'wordbank' && <WordBank />}
